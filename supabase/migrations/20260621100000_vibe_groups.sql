CREATE OR REPLACE FUNCTION public.generate_smart_shuffle(playlist_size integer DEFAULT 25, target_genre text DEFAULT NULL)
...
AS $function$
  WITH filtered_tracks AS (
    ...
    WHERE (
       ...
       OR target_genre = 'all'
       OR (
          target_genre = 'schranz' AND EXISTS (
            SELECT 1 FROM unnest(t.genre_tags) g
            WHERE g ILIKE ANY(ARRAY['%schranz%', '%hard techno%', '%acid%', '%dark techno%', '%emotional techno%'])
          )
       )
       OR (
          target_genre = 'trance' AND EXISTS (
            SELECT 1 FROM unnest(t.genre_tags) g
            WHERE g ILIKE ANY(ARRAY['%trance%', '%psytrance%'])
          )
       )
       OR (
          target_genre NOT IN ('schranz', 'trance') AND EXISTS (
            SELECT 1 FROM unnest(t.genre_tags) g 
